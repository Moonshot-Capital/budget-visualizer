...
(() => {
    ...
    const COMMON = {
        de: { blogLink: 'Blog', feedbackLink: 'Feedback', supportLink: 'Projekt unterst\u00fctzen', navApp: 'Visualizer', navHome: 'Startseite', howItWorks: 'So funktioniert\u2019s', backLink: '\u2190 Visualizer', openApp: 'Visualizer \u00f6ffnen', footerNote: 'Kostenlose Beta \u2013 ohne Konto, ohne Werbung. Alles bleibt in deinem Browser.' },
        en: { blogLink: 'Blog', feedbackLink: 'Feedback', supportLink: 'Support the project', navApp: 'Visualizer', navHome: 'Home', howItWorks: 'How it works', backLink: '\u2190 Visualizer', openApp: 'Open the visualizer', footerNote: 'Free beta \u2013 no account, no ads. Everything stays in your browser.' }
    };
    const PAGE = window.PAGE_TR || { de: {}, en: {} };
    let lang = ['de', 'en'].includes(state.lang) ? state.lang : 'de';

    const has = (k) => (PAGE[lang] && PAGE[lang][k] !== undefined) || COMMON[lang][k] !== undefined;
    const t = (k) => (PAGE[lang] && PAGE[lang][k]) ?? COMMON[lang][k] ?? k;
    function apply() {
        document.documentElement.lang = lang;
        document.querySelectorAll('[data-i18n]').forEach(el => { if (has(el.dataset.i18n)) el.textContent = t(el.dataset.i18n); });
